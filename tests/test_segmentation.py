...
def test_net29_established_return_traffic_allowed():
    ...
    assert_traffic_hits_rule("clab-soc-a3-d2-users", "10.51.50.10", 443, "tcp", "stateful-return")
...
def test_net07_management_can_reach_gateway_ssh():
    ...
    assert_traffic_hits_rule("clab-soc-a3-d2-management", "10.51.10.1", 22, "tcp", "mgmt-to-gateway-ssh")


def test_net08_users_cannot_reach_gateway_ssh():
    """NET-08: users -> network devices (the gateway itself), tcp/22. Expected: deny."""
    assert_traffic_hits_rule("clab-soc-a3-d2-users", "10.51.40.1", 22, "tcp", "NF_INPUT_DENY")
...
def test_net28_guest_spoofed_source_detected():
    ...
    send_spoofed_packet("clab-soc-a3-d2-guest", "10.51.10.99", "10.51.10.10", 53, protocol="udp")
...
def test_d1_finance_established_return_blocked():
    ...
    run_in("clab-soc-a3-d2-finance", ["nc", "-zv", "-w3", "10.51.50.10", "443"], timeout=6)
    time.sleep(0.5)
...
def test_fault2_users_should_not_reach_finance_admin():
    ...
    run_in("clab-soc-a3-d2-users", ["nc", "-zv", "-w3", "10.51.20.10", "22"], timeout=6)
    time.sleep(0.5)
...
def test_d2_dmz_visibility_restored():
    ...
    run_in("clab-soc-a3-d2-dmz", ["nc", "-zv", "-w3", "10.51.50.10", "5432"], timeout=6)
    count = wait_for_new_flow("10.51.60.10", "10.51.50.10", since=since)
